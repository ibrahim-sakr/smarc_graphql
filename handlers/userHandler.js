...
const find = async (parentValue, args, context) => {
    ...
    return await mongo.db().collection('users').findOne({ _id: mongo.id.new(args._id) });
};
...
const create = async (parentValue, args, context) => {
    ...
    const insertedDocuments = await mongo.db().collection('users').insertOne(args.user);
...
const update = async (parentValue, args, context) => {
    ...
    const updatedDocuments = await mongo.db().collection('users').findOneAndUpdate({ _id: mongo.id.new(args._id) }, {
        $set: args.device
    });
...
const deletion = async (parentValue, args, context) => {
    ...
    const deletedDocuments = await mongo.db().collection('users').findOneAndDelete({ _id: mongo.id.new(args._id) });
